...
interface StudentCourseRecordedCurriculumProps {
...
function RecordedLessonRow({
  ...
  const LessonIcon = isCompleted ? CheckCircle2 : RECORDED_LESSON_ICONS[lesson.type].icon;
...
export function StudentCourseRecordedCurriculum({
  modules,
  courseSlug,
  activeLessonId,
  completedTopics,
  totalTopics,
  progressPercent,
}: StudentCourseRecordedCurriculumProps) {
    ...
    <div className="rounded-2xl border border-[#ebe8e6] bg-white p-5 shadow-[0_8px_30px_rgba(35,25,22,0.06)] sm:p-6">
      ...
      <div className="mt-4 space-y-2.5">
        <p className="text-[13px] text-[#9ca3af]">
          <span className="font-semibold text-[#1a1a1a]">{completedTopics}</span> of{" "}
          <span className="font-semibold text-[#1a1a1a]">{totalTopics}</span> topics have been done
        </p>
