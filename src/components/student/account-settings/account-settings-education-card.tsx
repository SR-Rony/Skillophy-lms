...
import type { StudentAccountSettingsEducationItem } from "@/types/student-account-settings.types";
import { AccountSettingsStatusIndicator } from "./account-settings-status-indicator";
import { cn } from "@/utils";

interface AccountSettingsEducationItemRowProps {
  item: StudentAccountSettingsEducationItem;
  className?: string;
}

export function AccountSettingsEducationItemRow({
  item,
  className,
}: AccountSettingsEducationItemRowProps) {
  return (
...
interface AccountSettingsEducationCardProps {
  items: StudentAccountSettingsEducationItem[];
  className?: string;
}

export function AccountSettingsEducationCard({
  items,
  className,
}: AccountSettingsEducationCardProps) {
  return (
    <div className={cn("space-y-6", className)}>
      {items.map((item, index) => (
        <div key={item.id}>
          <AccountSettingsEducationItemRow item={item} />
