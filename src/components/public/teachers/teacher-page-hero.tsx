...
import { teacherPageHeroData } from "@/components/public/teachers/data/teacher-page-hero.data";
...
export function TeacherPageHero() {
  const { label, title, description, ctaLabel, imageSrc, imageAlt } = teacherPageHeroData;
      ...
      >
        <div className="grid items-center gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:gap-14 xl:gap-16">
          <motion.div variants={fadeUpVariants} className="relative max-w-[560px]">
            ...
            <div className="mt-9">
              <Button asChild variant="publicCta" size="publicCta">
                <Link href={ROUTES.auth.register}>{ctaLabel}</Link>
              </Button>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="relative aspect-[4/5] w-full max-w-[420px] sm:max-w-[480px] lg:max-w-[520px]">
              <div className="absolute -inset-3 rounded-[40px] bg-gradient-to-br from-primary/10 via-[#ffe2cc]/20 to-[#e8dff5]/25 blur-sm" />
