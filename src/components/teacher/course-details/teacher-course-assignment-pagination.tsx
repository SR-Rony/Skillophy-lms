...
interface TeacherCourseAssignmentPaginationProps {
  currentPage: number;
  totalPages: number;
...
export function TeacherCourseAssignmentPagination({
  currentPage,
  totalPages,
  onPageChange,
  className,
}: TeacherCourseAssignmentPaginationProps) {
...
export const TEACHER_COURSE_ASSIGNMENT_PAGE_SIZE = 7;

export function getTeacherCourseAssignmentPaginationMeta(totalItems: number, currentPage: number) {
  const totalPages = Math.max(1, Math.ceil(totalItems / TEACHER_COURSE_ASSIGNMENT_PAGE_SIZE));
  const safePage = Math.min(Math.max(1, currentPage), totalPages);
  const startIndex = (safePage - 1) * TEACHER_COURSE_ASSIGNMENT_PAGE_SIZE;

  return {
    totalPages,
    currentPage: safePage,
    startIndex,
    endIndex: startIndex + TEACHER_COURSE_ASSIGNMENT_PAGE_SIZE,
  };
}
