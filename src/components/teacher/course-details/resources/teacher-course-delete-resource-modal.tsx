...
interface TeacherCourseDeleteResourceModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  material: TeacherCourseResourceItem | null;
  onConfirm?: (material: TeacherCourseResourceItem) => void;
}

export function TeacherCourseDeleteResourceModal({
  open,
  onOpenChange,
  material,
  onConfirm,
}: TeacherCourseDeleteResourceModalProps) {
        ...
        <Dialog.Content className="teacher-center-modal-panel fixed left-1/2 top-1/2 z-[60] w-[calc(100%-2rem)] max-w-[520px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-[0_24px_60px_rgba(0,0,0,0.25)] focus:outline-none sm:p-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <Dialog.Close
              ...
              aria-label="Close delete resource modal"
            >
          ...
          <Dialog.Description asChild>
            <p className="mt-5 text-[14px] leading-relaxed text-[#6b7280] sm:mt-6 sm:text-[15px]">
              Are you sure you want to delete the resource file{" "}
              <span className="font-bold text-[#1a1a1a]">{material?.title ?? "this file"}</span>?
            </p>
          </Dialog.Description>
          ...
          <div className="mt-7 sm:mt-8">
            <button
              ...
              disabled={!material}
              className="inline-flex min-w-[140px] items-center justify-center rounded-xl bg-primary px-6 py-3 text-[14px] font-bold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 sm:text-[15px]"
            >
              Delete Resource
            </button>
          </div>
