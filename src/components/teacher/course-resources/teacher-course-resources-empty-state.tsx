...
function TeacherCourseResourcesEmptyIllustration() {
  return (
...
interface TeacherCourseResourcesEmptyStateProps {
  heading: string;
  message: string;
  actionLabel: string;
  onAddResources: () => void;
...
export function TeacherCourseResourcesEmptyState({
  heading,
  message,
  actionLabel,
  onAddResources,
}: TeacherCourseResourcesEmptyStateProps) {
  return (
    <div className="flex flex-col items-center px-4 py-14 text-center sm:py-16 md:py-20">
      <TeacherCourseResourcesEmptyIllustration />
      <h2 className="mt-8 text-[22px] font-bold text-[#1a1a1a] sm:text-[24px]">{heading}</h2>
      <p className="mt-2 max-w-lg text-sm leading-relaxed text-[#9ca3af] sm:text-[15px]">{message}</p>
      <button
        type="button"
