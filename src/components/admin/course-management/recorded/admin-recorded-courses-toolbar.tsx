...
import type {
  ...
  AdminRecordedCourseCategoryOption,
  AdminRecordedCourseExportId,
  AdminRecordedCourseExportOption,
  AdminRecordedCourseSortId,
  AdminRecordedCourseSortOption,
} from "@/types/admin-course-management.types";
import { cn } from "@/utils";

interface AdminRecordedCoursesToolbarProps {
  searchQuery: string;
  ...
  selectedCategoryId: AdminRecordedCourseCategoryId;
  selectedSortId: AdminRecordedCourseSortId;
  exportLabel: string;
  addNewLabel: string;
  onSearchChange: (value: string) => void;
  onCategoryChange: (categoryId: AdminRecordedCourseCategoryId) => void;
  onSortChange: (sortId: AdminRecordedCourseSortId) => void;
  onExport: (exportId: AdminRecordedCourseExportId) => void;
  onAddNew: () => void;
}
...
function ExportOptionIcon({
  ...
  exportId: AdminRecordedCourseExportId;
  className?: string;
}) {
  if (exportId === "csv") {
...
export function AdminRecordedCoursesToolbar({
  searchQuery,
  categoryOptions,
  sortOptions,
  exportOptions,
  ...
  onExport,
  onAddNew,
}: AdminRecordedCoursesToolbarProps) {
  ...
  const selectedCategoryLabel =
    categoryOptions.find((option) => option.id === selectedCategoryId)?.label ?? "All Category";
    ...
    <div className="border-b border-[#f0f0f0] bg-white px-4 py-4 sm:px-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div ref={categoryDropdown.containerRef} className="relative shrink-0">
            <div className={toolbarBoxClassName}>
              <button
                type="button"
                id={categoryMenuId}
                aria-haspopup="listbox"
                aria-expanded={categoryDropdown.open}
                onClick={() => categoryDropdown.setOpen((current) => !current)}
              ...
              <button
                type="button"
                aria-labelledby={categoryMenuId}
                onClick={() => categoryDropdown.setOpen((current) => !current)}
        ...
        <div className="flex shrink-0 items-center gap-2.5 sm:gap-3 xl:ml-auto">
          ...
          <button
            type="button"
            onClick={onAddNew}
            className="inline-flex h-10 items-center gap-2 rounded-xl bg-primary px-4 text-[13px] font-semibold text-white transition-colors hover:bg-[#e63e3e] active:bg-[#d93636]"
          >
            {addNewLabel}
