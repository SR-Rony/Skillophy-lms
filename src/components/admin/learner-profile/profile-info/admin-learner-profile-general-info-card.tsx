...
import type {
  AdminLearnerProfileGeneralInfo,
} from "@/types/admin-learner-profile.types";
import type { StudentAccountSettingsSelectOption } from "@/types/student-account-settings.types";
import { cn } from "@/utils";

interface AdminLearnerProfileGeneralInfoCardProps {
...
export function AdminLearnerProfileGeneralInfoCard({
  generalInfo,
  genderOptions,
  countryOptions,
  isEditing,
  onEditingChange,
  className,
}: AdminLearnerProfileGeneralInfoCardProps) {
  const [form, setForm] = useState(generalInfo);
  const [savedForm, setSavedForm] = useState(generalInfo);

  function handleChange(field: keyof AdminLearnerProfileGeneralInfo, value: string) {
