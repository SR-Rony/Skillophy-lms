...
import type {
  AdminLearnerExportId,
  AdminLearnerExportOption,
  AdminLearnerSortId,
  AdminLearnerSortOption,
} from "@/types/admin-learner-management.types";
import { cn } from "@/utils";

interface AdminLearnerManagementToolbarProps {
  searchQuery: string;
  sortOptions: AdminLearnerSortOption[];
  exportOptions: AdminLearnerExportOption[];
  selectedSortId: AdminLearnerSortId;
  exportLabel: string;
  onSearchChange: (value: string) => void;
  onSortChange: (sortId: AdminLearnerSortId) => void;
  onExport: (exportId: AdminLearnerExportId) => void;
}
...
function ExportOptionIcon({ exportId, className }: { exportId: AdminLearnerExportId; className?: string }) {
  if (exportId === "csv") {
...
export function AdminLearnerManagementToolbar({
  searchQuery,
  sortOptions,
  exportOptions,
  selectedSortId,
  ...
  onSortChange,
  onExport,
}: AdminLearnerManagementToolbarProps) {
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-[#f0f0f0] bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      ...
      <div ref={exportDropdown.containerRef} className="relative shrink-0">
        <button
          type="button"
          id={exportMenuId}
          aria-haspopup="menu"
