...
import type {
  AdminTemplateSortId,
  AdminTemplateSortOption,
  AdminTemplateTypeFilterId,
  AdminTemplateTypeOption,
} from "@/types/admin-template-management.types";
import { cn } from "@/utils";

interface AdminTemplateManagementToolbarProps {
  searchQuery: string;
  typeOptions: AdminTemplateTypeOption[];
  sortOptions: AdminTemplateSortOption[];
  selectedTypeId: AdminTemplateTypeFilterId;
  selectedSortId: AdminTemplateSortId;
  addNewLabel: string;
  onSearchChange: (value: string) => void;
  onTypeChange: (typeId: AdminTemplateTypeFilterId) => void;
  onSortChange: (sortId: AdminTemplateSortId) => void;
  onAddNew: () => void;
}
...
export function AdminTemplateManagementToolbar({
  searchQuery,
  typeOptions,
  sortOptions,
  selectedTypeId,
  ...
  onSortChange,
  onAddNew,
}: AdminTemplateManagementToolbarProps) {
    ...
    <div className="border-b border-[#f0f0f0] bg-white px-4 py-4 sm:px-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div ref={typeDropdown.containerRef} className="relative shrink-0">
            <div className={toolbarBoxClassName}>
              ...
              >
                <span className="text-[13px] font-medium text-[#4a4a4a]">{selectedTypeLabel}</span>
                <ChevronDown
                  className={cn(
            ...
            {typeDropdown.open && (
              <ul
                className="absolute left-0 top-[calc(100%+8px)] z-30 min-w-[240px] overflow-hidden rounded-xl border border-[#ebe8e6] bg-white py-2 shadow-[0_10px_30px_rgba(35,25,22,0.08)]"
                role="listbox"
              >
