...
import type {
  AdminTeacherPaymentExportId,
  AdminTeacherPaymentExportOption,
  AdminTeacherPaymentSortId,
  AdminTeacherPaymentSortOption,
} from "@/types/admin-teacher-profile.types";
import { cn } from "@/utils";

interface AdminTeacherProfilePaymentToolbarProps {
  searchQuery: string;
  sortOptions: AdminTeacherPaymentSortOption[];
  exportOptions: AdminTeacherPaymentExportOption[];
  selectedSortId: AdminTeacherPaymentSortId;
  exportLabel: string;
  markAsPaidLabel: string;
  canMarkAsPaid: boolean;
  onSearchChange: (value: string) => void;
  onSortChange: (sortId: AdminTeacherPaymentSortId) => void;
  onExport: (exportId: AdminTeacherPaymentExportId) => void;
  onMarkAsPaid: () => void;
}
...
function ExportOptionIcon({
  ...
  exportId: AdminTeacherPaymentExportId;
  className?: string;
}) {
  if (exportId === "csv") {
...
export function AdminTeacherProfilePaymentToolbar({
  searchQuery,
  sortOptions,
  exportOptions,
  selectedSortId,
  ...
  onExport,
  onMarkAsPaid,
}: AdminTeacherProfilePaymentToolbarProps) {
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-[#f0f0f0] px-4 py-4 lg:flex-row lg:items-center lg:justify-between sm:px-6">
      ...
      <div className="flex shrink-0 items-center gap-2.5 sm:gap-3">
        ...
        <button
          ...
          aria-disabled={!canMarkAsPaid}
          className="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl bg-primary px-5 text-[13px] font-semibold text-white shadow-[0_2px_8px_rgba(255,71,71,0.25)] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
        >
