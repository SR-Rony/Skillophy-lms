...
import type {
  AdminEmployeeExportId,
  AdminEmployeeExportOption,
  AdminEmployeeRoleOption,
  AdminEmployeeSortId,
  AdminEmployeeSortOption,
} from "@/types/admin-employee-management.types";
import { cn } from "@/utils";

interface AdminEmployeeManagementToolbarProps {
  searchQuery: string;
  roleOptions: AdminEmployeeRoleOption[];
  sortOptions: AdminEmployeeSortOption[];
  exportOptions: AdminEmployeeExportOption[];
  selectedRoleId: string;
  selectedSortId: AdminEmployeeSortId;
  exportLabel: string;
  addNewLabel: string;
  onSearchChange: (value: string) => void;
  onRoleChange: (roleId: string) => void;
  onSortChange: (sortId: AdminEmployeeSortId) => void;
  onExport: (exportId: AdminEmployeeExportId) => void;
  onAddNew: () => void;
}
...
function ExportOptionIcon({ exportId, className }: { exportId: AdminEmployeeExportId; className?: string }) {
  if (exportId === "csv") {
...
export function AdminEmployeeManagementToolbar({
  searchQuery,
  roleOptions,
  sortOptions,
  exportOptions,
  ...
  onExport,
  onAddNew,
}: AdminEmployeeManagementToolbarProps) {
  ...
  const selectedRoleLabel =
    roleOptions.find((option) => option.id === selectedRoleId)?.label ?? "All Role";
  const selectedSortLabel =
    sortOptions.find((option) => option.id === selectedSortId)?.label ?? "Default";

  return (
    <div className="flex flex-col gap-3 border-b border-[#f0f0f0] px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:max-w-[620px]">
        ...
        <div ref={roleDropdown.containerRef} className="relative shrink-0">
          <div className={toolbarBoxClassName}>
            <button
              ...
              className="flex items-center gap-2.5 border-r border-[#ebe8e6] px-3.5"
            >
            ...
            >
              <span className="text-[13px] font-semibold text-[#1a1a1a]">{selectedRoleLabel}</span>
              <ChevronDown
                className={cn(
      ...
      <div className="flex items-center gap-2.5 sm:gap-3">
        <div ref={exportDropdown.containerRef} className="relative">
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-xl border border-[#ebe8e6] bg-[#f5f5f4] px-4 text-[13px] font-semibold text-[#1a1a1a] transition-colors hover:bg-[#ececec]"
          >
            {exportLabel}
            <ChevronDown
              className={cn(
            ...
            >
              {exportOptions.map((option) => (
                <li key={option.id} role="none">
                  ...
                  >
                    <ExportOptionIcon exportId={option.id} className="h-4 w-4 shrink-0 text-[#6b7280]" />
                    {option.label}
                  </button>
                </li>
              ))}
        ...
        <button
          type="button"
          onClick={onAddNew}
          className="inline-flex h-10 items-center rounded-xl bg-primary px-4 text-[13px] font-semibold text-white transition-colors hover:bg-primary/90"
        >
          {addNewLabel}
        </button>
      </div>
