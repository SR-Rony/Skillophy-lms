...
interface AdminDeleteMemberModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  employee: AdminEmployee | null;
  onConfirm?: (employee: AdminEmployee) => void;
}

export function AdminDeleteMemberModal({
  open,
  onOpenChange,
  employee,
  onConfirm,
}: AdminDeleteMemberModalProps) {
        ...
        <Dialog.Content className="teacher-center-modal-panel fixed left-1/2 top-1/2 z-[60] w-[calc(100%-2rem)] max-w-[520px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-[0_24px_60px_rgba(0,0,0,0.25)] focus:outline-none sm:p-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <Dialog.Close
              ...
              aria-label="Close delete member modal"
            >
          ...
          <Dialog.Description asChild>
            <p className="mt-5 text-[14px] leading-relaxed text-[#6b7280] sm:mt-6 sm:text-[15px]">
              ...
              <span className="font-bold text-[#1a1a1a]">{employee?.name ?? "this member"}</span>?
            </p>
          </Dialog.Description>
