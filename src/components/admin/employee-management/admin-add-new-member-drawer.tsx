...
import type { AdminEmployeeInviteRoleOption } from "@/types/admin-employee-management.types";
import { cn } from "@/utils";

interface AdminAddNewMemberDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  roleOptions: AdminEmployeeInviteRoleOption[];
  defaultEmail: string;
  defaultRoleId: string;
  onSendRequest?: (payload: { email: string; roleId: string }) => void;
}

export function AdminAddNewMemberDrawer({
  open,
  onOpenChange,
  roleOptions,
  defaultEmail,
  ...
  }, [open, defaultEmail, defaultRoleId]);
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    onSendRequest?.({ email: email.trim(), roleId });
    onOpenChange(false);
  }
        ...
        >
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 pr-2">
                ...
                <Dialog.Description className="mt-2 text-[13px] leading-relaxed text-[#9ca3af] sm:text-[14px]">
                  Please specify the new member&apos;s email address and the appropriate role for
                  their access within the portal.
                </Dialog.Description>
              </div>
          ...
          <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="border-t border-[#f0ebe8] px-5 py-5 sm:px-6 sm:py-6">
              <button
                type="submit"
                className="inline-flex min-w-[148px] items-center justify-center rounded-xl bg-primary px-8 py-3 text-[14px] font-bold text-white transition-opacity hover:opacity-90 sm:text-[15px]"
              >
                Send Request
              </button>
            </div>
