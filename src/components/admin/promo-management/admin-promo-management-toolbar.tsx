...
interface AdminPromoManagementToolbarProps {
  searchQuery: string;
  categoryOptions: AdminPromoCategoryOption[];
  sortOptions: AdminPromoSortOption[];
  selectedCategoryId: AdminPromoCategoryId;
  selectedSortId: AdminPromoSortId;
  addNewLabel: string;
  onSearchChange: (value: string) => void;
  onCategoryChange: (categoryId: AdminPromoCategoryId) => void;
  onSortChange: (sortId: AdminPromoSortId) => void;
  onAddNew: () => void;
}
...
function SortOptionIcon({ sortId, className }: { sortId: string; className?: string }) {
  if (sortId.includes("asc") && sortId !== "discount-desc") {
...
export function AdminPromoManagementToolbar({
  searchQuery,
  categoryOptions,
  sortOptions,
  selectedCategoryId,
  ...
  onSortChange,
  onAddNew,
}: AdminPromoManagementToolbarProps) {
    ...
    <div className="border-b border-[#f0f0f0] bg-white px-4 py-4 sm:px-6">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        ...
        <button
          type="button"
          onClick={onAddNew}
          className="inline-flex h-10 shrink-0 items-center gap-2 self-start rounded-xl bg-primary px-4 text-[13px] font-semibold text-white transition-colors hover:bg-[#e63e3e] active:bg-[#d93636] xl:ml-auto xl:self-auto"
        >
          {addNewLabel}
